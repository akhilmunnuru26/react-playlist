...
const App = () => {
  ...
  const searchTracks = event => {
    if (event.target.value !== '') {
      const filteredTrackList = tracksList.filter(eachItem =>
        eachItem.name.toLowerCase().includes(event.target.value.toLowerCase()),
      )
      setTracksList(filteredTrackList)
    } else {
      setTracksList(initialTracksList)
    }
  }

  const getTrackId = id => {
    const filteredTrackList = tracksList.filter(eachItem => eachItem.id !== id)
    setTracksList(filteredTrackList)
  }
    ...
    <div className="bg-container">
      ...
      <div className="music-section">
        <div className="music-section-header">
          <h1 className="music-section-header-title">Songs Playlist</h1>
          <div className="search-container">
            <input
              ...
              type="search"
            />
            <AiOutlineSearch font-size="20" color="#cbd5e1" />
          </div>
        </div>
        {tracksList.length !== 0 ? (
          ...
          <div className="no-songs-found">
            <p className="no-songs-found-msg">No Songs Found</p>
          </div>
        )}
      </div>
    </div>
